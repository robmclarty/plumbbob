...
// back. Step 2 lands too: the typed range is the human's approval for it, so
// the turn ends on its checkpoint rather than holding it at a pause. (Plan 05,
// contract table row 4.) In the latched sweep the driver pre-arms `range 2`
  ...
  async run(session, fixture) {
    ...
    return {
      ...
      checks: [
        ...
        // The top step lands like the rest, so the turn ends on its checkpoint
        // block: the boundary, with nothing left pending to decide.
