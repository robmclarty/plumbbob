...
- `lib/` — the shared mechanics: `git.ts` (additive-only git, C5), `sidecar.ts` (paths,
  planes, excludes), `intent.ts` (step/seam parsing), `orient.ts` (the dashboard),
  `settings.ts` (the ladder, D27), `check.ts` (the gate), `buildlog.ts`, `plugins.ts`
  (install detection).

Structural invariants are machine-enforced by the ast-grep rules in `rules/`:
functional/procedural only (C1), node builtins plus the dependency allowlist (C2),
subprocess spawning centralized, output through `process.stdout`/`stderr` only.

## The two planes (D17/D26)
...
ephemera, excluded via the shared gitdir's `info/exclude` (D33), never `.gitignore`:
`STATE`, `settings.local.json` (holding the `activeBuild` cursor, D28), and each
