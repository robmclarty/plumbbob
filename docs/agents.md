...
PlumbBob's executor is **author-blind** (**D3**): `/pb-verify` and `checkpoint` read *the
diff, not who wrote it*, so a step's code can come from you, from `/pb-build`, from a vibe
...
want to run the loop, you never need this page; the [`/pb-*` skills](skills-reference.md)
...
1. **The envelope has no verb to advance the loop** (**C6** — the identity invariant).
...
2. **The CLI owns every side effect** (**D44**). An agent *returns* concerns; PlumbBob
...
3. **Review is advisory; checkride gates; the human advances** (**D45**). An
...
- **exit code** — `0` means the envelope on stdout is authoritative. **Any non-zero exit
  ...
  child that failed. This is the attention-first split (**D46**): production narrates on
...
- `step.seam` is parsed **strictly** (it gates git behavior): exact paths or `dir/` grants,
  ...
  prose** scraped from `intent.md` (**D61**): a formatting quirk warns on stderr and is
  ...
  the ruling.
- `context[]` carries the output of `before`-slot agents, threaded in inline (**D59**) — how
...
- `settings` is *plumbbob's* relevant configuration only. Provider keys, model choice, and
  sandboxing are **your** agent's business (**D53**) — its env, its config; PlumbBob never
  touches a key.
...
| `drift`   | Finished, but the plan no longer matches reality. | Repair the plan with `/pb-refine` before continuing. |

`parked[]` is how an agent captures a mid-run "ooh, what if" without acting on it — each
string becomes a park line the CLI lands through the park verb (**D44**). Unknown fields are
tolerated and dropped (**C7**, see Versioning); a malformed `parked` (non-strings,
...
The manifest speaks to **two audiences** (**D55**):
...
### How the command runs (**D49**, POSIX only)
...
Two tiers, plus a flag, resolved first-hit-wins (**D41** — the settings ladder's shape, and
...
warning** and the loop runs without it (**D54**) — the same optionality contract as
...
of `intent.md` under `builds/<slug>/`, authored at `/pb-plan` time (**D42**). Three slots,
and only three (**D43**):
...
middle of the work" (**D43**). That's judgment, and it's handled in *prose* (a manifest's
...
- **The merge ladder** (**D57**): for a given step and slot, a `--agent` flag beats a per-step
...
- **`harness.json` stays bindings + prose only** (**C3**). The moment it grows an `if`, a
...
step-scoped, cleared when the step checkpoints, **D47**) so a later `agent run` or a
...
The two ways an agent can go missing route differently (**D54**):
...
- **A harness binding names it** and a teammate lacks it (personal-tier only): **warns and
  skips** (**D54**). A binding is ambient configuration the loop must survive without.

A run that actually *starts* and then fails — non-zero exit, a timeout, garbage on stdout —
is a hard failure either way. D54 softens a *missing* agent, not a *broken* one.

### Interrupts and timeouts

- **Ctrl-C** is forwarded to the child and kills it, then reports — a present human's
  interrupt never orphans the agent (**D58**).
- **Timeouts are off by default** (**D51**). Set `agentTimeout` (seconds) in the settings
...
a cutoff, say (**D50**). This is allowed with no environment guard, because:

- **Loops belong inside agents, as code.** A build-then-review-until-clean loop is legitimate
  composition; its cutoff is *your* job as the author (config never grew a `retry` — **C3**).
- **The identity invariant holds at every depth.** The envelope has no verb to advance
  PlumbBob's loop (**C6**), so no nesting depth can smuggle one in. A composed agent can drive
...
side is behind (**D46**), because across a major the shapes may genuinely disagree. Keep your
envelope **minimal** — resist field sprawl (SWE-agent's ACI lesson): additions are a minor
bump, removals or renames are a major (**C7**).
...
- [`skills-reference.md`](skills-reference.md#the-harness-slots) — how `/pb-plan`, `/pb-step`,
  `/pb-build`, and `/pb-verify` drive the slots for you.
