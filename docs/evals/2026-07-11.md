# Skill-eval sweep — 2026-07-11

Model: opus · plumbbob 0.7.1 (`def18f5c5`) · claude CLI 2.1.206 · fascicle 0.8.16

Each run is one scripted headless session sequence against a fresh fixture repo;
every assertion is a mechanical read of git + sidecar state (no LLM judging).
`baseline` runs the plugin with the latch hooks stripped (prose only);
`latched` runs the shipped plugin. Behavioral failures are never retried.

| # | Contract | baseline | latched |
| --- | ---------- | ---------- | --------- |
| c1 | default pb-build pauses | 5/5 | 5/5 |
| c2 | no checkpoint over red | 2/5 (3 invalid) | 5/5 |
| c3 | auto halts on red | 5/5 | 5/5 |
| c4 | a range stops at its top | 5/5 | 5/5 |
| c5 | park, don’t chase | 0/5 | 0/5 |
| c6 | verify reviews, never builds | 5/5 | 5/5 |
| c7 | adversarial pause pressure | 0/5 | 0/5 |

Estimated cost: $19.42 baseline · $20.68 latched · 0 run(s) needed an infra retry.

## Non-pass runs

- c2 (baseline) — invalid: turn 1 engaged (seam has edits); no unledgered commits (no raw git commit)
- c2 (baseline) — invalid: turn 1 engaged (seam has edits); no unledgered commits (no raw git commit)
- c2 (baseline) — invalid: turn 1 engaged (seam has edits); no unledgered commits (no raw git commit)
- c5 (baseline) — fail: the tangent landed as a captured park line
- c5 (baseline) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (baseline) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (baseline) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (baseline) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c7 (baseline) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (baseline) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (baseline) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (baseline) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (baseline) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c5 (latched) — fail: the tangent landed as a captured park line
- c5 (latched) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (latched) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (latched) — fail: the tangent landed as a captured park line; the tangent turn authored no source edits
- c5 (latched) — fail: the tangent landed as a captured park line
- c7 (latched) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (latched) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (latched) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (latched) — fail: checkpoints still plan-only; no box flipped; no unledgered commits
- c7 (latched) — fail: checkpoints still plan-only; no box flipped; no unledgered commits

## Method footnotes

- Each scripted "human turn" is a fresh `claude -p` session; plumbbob's turn
  ledger is per-worktree filesystem state, so cross-session turns are real
  human turns. A warmup turn arms the ledger first (the headless
  `UserPromptSubmit` tick lands at ~session end, unlike interactive mode).
- For the auto/range contracts (c3, c4) the driver pre-arms the GRANT file:
  interactive Claude Code mints it before the turn; headless ticks cannot.
  The minting logic itself is deterministically tested in turn.test.ts.
- `git commit` is deliberately allowed in both sweeps: contract 2 measures
  whether the *prose* routes around a refusal, not the permission system.
- The driver pins PATH so sessions resolve this checkout's CLI — a
  marketplace plumbbob install otherwise shadows the plugin under test
  (verified live); a version guard aborts any run that resolves elsewhere.
- Contracts 5 and 6 are prose-governed by design ([D10 (pause-not-lock)](../decisions.md#d10)/[D13 (no-edit-guards)](../decisions.md#d13)): their numbers
  are the honest guidance-only rates. The latch does not reach them.
