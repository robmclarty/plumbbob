...
it first whenever a `/pb-*` skill misbehaves.

## Install and linking

### A `/pb-*` skill opens with an empty dashboard
...
mid-step (`BUILD`): finish the step with `/pb-verify` first, then harvest from
`DESIGN`.
...
- **No session.** The hook is session-gated: with no `.plumbbob/STATE`, the repo behaves
  like plain Claude Code. Start a session to enable it.
