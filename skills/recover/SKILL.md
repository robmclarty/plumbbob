...
model: haiku
allowed-tools: Bash(plumbbob status:*), Bash(plumbbob recover:*), Bash(plumbbob handoff:*)
---
...
This is a **driver skill**: a chat-side trigger for the mechanical `plumbbob recover` verb, so the whole workflow runs from the agent window instead of a terminal. It is `disable-model-invocation: true`: only the human fires it. It carries **no Edit and no Write tool**; it shells the verb, relays its output verbatim (including any refusal), and closes on `plumbbob handoff`'s next-up pointer for the turn. The CLI is the source of truth: never retry a refused transition, and never edit a file to work around one.
...
4. Close on `plumbbob handoff --driver`'s next-up line (the driver tier of the [turn anatomy](https://github.com/robmclarty/plumbbob/blob/main/docs/presentation.md)): a readout that just told the human their state was off ends on one pointer to reorient them, back at the open step when one survives and forward from the boundary otherwise.
