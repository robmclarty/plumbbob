...
`/plan` authors the **whole** step list up front, so `/step` is the
**just-in-time revision** move: it keeps the *next* undone step honest against what the
build has actually taught you, right before you `/build` it. (Framing the whole goal
is the separate `/plan` move.) It can also add or re-cut a step when scope genuinely
grew — but its everyday job is to sharpen, not to invent.

## Two ways to fire it

- **`/step` (no input) → automatic sharpen.** Re-examine the next undone step
...
- **`/step <what changed>` → directed revision.** Take the human's input and propose
...
2. **Propose the revision** (or the new/split step): a one-line **title**, a **done-when**
   `/verify` can validate, and a **seam** (exact paths, or a `dir/` grant). Keep it
...
4. **Revise the step's harness bindings if they drifted too** *(optional)*. If the
   ...
   counterpart to `/plan`'s plan-time binding. `plumbbob agent list` shows what's
...
- **One verifiable increment.** Each step carries a done-when `/verify` can check
