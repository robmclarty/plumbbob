...
13. [ ] chore(deps): upgrade checkride to 0.12.1 — **done when:** `pnpm check` is green on 0.12.1, the injected `DoctorEnv` carries the new `readText` probe, and the always-on adapter list is re-verified against the new `prose` slot rather than assumed
    ...
    - notes: 0.12.1 also ships the `prose` slot and its `exemplars` key, which is the seam [D16 — voice-exemplars-are-hand-owned](#d16) was written for. Wiring it is [Q8 (prose-slot-wiring)](#open-questions), deliberately not this step.
...
- Q8 (prose-slot-wiring): does checkride's `prose` slot, with `exemplars` pointed at `docs/voice/`, get wired in this build or its own? — *resolve by:* decide
