...
- **Smallest thing that solves it:** The README's v1 surface, no more: the
  ...
  light post-edit feedback), the five skills, and an installer (global by
  default, repo-scoped per D27) — working on this machine, for one user,
  dogfooded on its own build.
...
- **Explicitly NOT doing:** Ridgeline integration; archive indexing/retrieval;
  ...
  multi-user; other-agent/editor adapters (Claude Code is v1's enforcement
  adapter — the CLI + `.plumbline/` sidecar core is agent- and model-agnostic;
  enforcement inherently means intercepting *some* agent's edit tool, so other
  adapters are a future Plumbline, not a v1 obligation); an adversarial-proof
  muzzle (it is a fence, not a wall — D21); spike timebox enforcement.
...
- D17: `.plumbline/` is untracked: `start` appends it to `.git/info/exclude`
  ...
  consequence is D20. Applies to this repo too: the bootstrap exception
  (sidecar committed with the first commits, preserving the founding plan in
  history) ended 2026-06-10 — `.plumbline/` is in this repo's own `.gitignore`
  and untracked from here on.
...
- D27: `plumbline setup` takes a registration scope: default global
  (`~/.claude/settings.json`); `--project` merges into
  `<repo>/.claude/settings.json` (committable — how a team repo enrolls every
  clone); `--local` into `<repo>/.claude/settings.local.json` (personal,
  untracked). Hooks and skills install once under `~/.claude/` in every scope
  — only where the registration lives varies — and repo-scoped entries write
  home-relative (`~`-prefixed) command paths, so committed settings carry no
  machine-absolute home dir; a teammate without Plumbline installed gets a
  non-blocking hook error, not a wall — *because* Claude Code merges hooks
  additively across its settings scopes and the hooks themselves are
  scope-agnostic (session-gated, generic — all per-repo state lives in the
  sidecar, none in settings), so scope is a sharing choice, not a behavior
  choice: global serves solo-across-many-repos; project scope is the only
  path to a repo enrolling its whole team.
- D28: The FINISH-entry verb keeps its working name `wrap`, resolving D19's
  deferred naming: `plumbline wrap` sets STATE=FINISH (so `/plumbline-report`
  can write `report.md` and `/plumbline-docs` can touch `docs/`); the report
  skill instructs you to run it; the closing gate stays `plumbline finish`. It
  lives in its own `src/verbs/wrap.ts` per the one-file-per-verb pattern (step
  6 seam updated) — *because* a named human-run transition earns the same
  dumb-CLI treatment as the other verbs, and keeping enter (`wrap`) and close
  (`finish`) as two verbs preserves the symmetric step/finish gate. Chosen
  "for now" and cheap to rename — no external surface depends on it yet.
...
1. [x] Toolchain bootstrap: heavy gate green, CLI stub, first commit —
   **done when:** `pnpm install` and `pnpm check` (tsc, oxlint, ast-grep,
   ...
   - seam: `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`,
     `.gitignore`, `tsconfig.json`, `vitest.config.ts`, `knip.json`,
     `.oxlintrc.json`, `sgconfig.yml`, `rules/no-class.yml`,
     `rules/no-default-export.yml`, `.markdownlint.jsonc`, `src/cli.ts`,
     `test/cli.test.ts`
     <!-- seam revised mid-step (sanctioned in-build seam rewrite): added
     pnpm-workspace.yaml — pnpm 11 gates dependency build scripts there via
     its `allowBuilds` map, required to make D24's gate run. No new decision;
     pure toolchain artifact of D15's pnpm choice. -->
2. [x] Sidecar + git lib and session verbs: `start`, `status`, `mode`, `park`
...
3. [x] Build-loop verbs: `build <n>`, `review`, `done`, `revert` —
   **done when:** vitest passes: the strict intent parser takes the nth item
   ...
   survive revert
   - seam: `src/lib/intent.ts`, `src/lib/check.ts`, `src/lib/git.ts`,
     `src/lib/sidecar.ts`, `src/verbs/build.ts`, `src/verbs/review.ts`,
     `src/verbs/done.ts`, `src/verbs/revert.ts`, `src/cli.ts`,
     `test/intent.test.ts`, `test/build-loop.test.ts`
     <!-- seam revised mid-step (sanctioned in-build seam rewrite): added
     src/lib/git.ts (mutation helpers: stageAll/stagedPaths/commit/resetHard/
     untrackedPaths for D8/C5) and src/lib/sidecar.ts (seamPath/stepPath for
     D4/D7). The build-loop extends the read-only step-2 libs; no new
     decision. -->
4. [x] Hooks: `pre-edit.sh` (muzzle+seam-guard combined), `bash-guard.sh`
...
6. [x] Finish phase + spike lifecycle: report gate, archive-then-clear, spike
   ...
   alongside the first; FINISH entry per D19/D28 (`wrap` verb) and spike
   lifecycle per D18 (sibling worktrees, main tree DESIGN-locked)
   - seam: `src/verbs/wrap.ts`, `src/verbs/finish.ts`, `src/verbs/spike.ts`,
     `src/lib/archive.ts`, `src/cli.ts`, `test/finish.test.ts`,
     `test/spike.test.ts`
7. [x] The five skills with enforced contracts — **done when:**
...
8. [x] Installer + e2e dogfood close-out — **done when:** `plumbline setup`
   (replacing dev-install) copies hooks to `~/.claude/plumbline/hooks/` and
   skills to `~/.claude/skills/`, merges the hook registration into the
   settings file D27's scope selects (default `~/.claude/settings.json`;
   `--project` ⇒ `<repo>/.claude/settings.json` with `~`-portable command
   paths; `--local` ⇒ `<repo>/.claude/settings.local.json`) idempotently
   (tested against a tmp HOME and a fixture repo: merge-into-existing-hooks,
   no-hooks-key, repeat-byte-identical, each scope writes only its own file),
   warns on PATH/restart; `test/e2e.test.ts` drives a
   ...
   README documents install (all three scopes), the as-built verb table, and
   the ratified residual gaps (D20 archive local-only, D21 fence-not-wall,
   D25 tsc deferral); this session itself is finished through its own gate
...
- 2026-06-11 — Setup registration scope (raised by the author between steps
  4 and 5): the spec's global-only `~/.claude/settings.json` registration was
  an unexamined assumption, never a weighed decision. Ruled with the author:
  `setup` gains `--project` / `--local` scope flags, global stays the default
  → D27. Step 8's done-when extended to cover all three scopes; seam
  unchanged (`src/verbs/setup.ts` + `src/lib/settings.ts` already cover it).
  No new step, no new open questions.
- 2026-06-11 — Step 5 closed under live enforcement (checkpoint `ffd1318`):
  the hooks are registered and the seam-guard was proven on the real repo (an
  out-of-seam `src/cli.ts` edit blocked in BUILD; code edits muzzled in
  DESIGN). D19's deferred FINISH-entry verb name was then resolved with the
  author at the step-6 boundary: keep the working name `wrap` (revisitable)
  → D28; step 6's seam gains `src/verbs/wrap.ts`. No new open questions.
