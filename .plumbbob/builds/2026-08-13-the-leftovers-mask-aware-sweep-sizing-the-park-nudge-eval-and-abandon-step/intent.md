...
4. [ ] fix(refs): scan scripts/ and cite only global decisions there, **done when:** `check-refs.ts`'s walker yields `scripts/**/*.ts` beside `src/**/*.ts` ([D16 (scripts-in-refs-scan)](#d16)); the ten violations the widened surface finds today are fixed at the source, the eight build-local tags (`D2` ×4, `D14` ×3, `D3` ×1) named in words under [D17 (global-tags-only-in-code)](#d17) and the two `D74` citations whose gloss wraps onto the next comment line reflowed so tag and gloss share a line; a `scanRepo` test pins that a `scripts/` file is walked; refs slot green; `pnpm check` green
