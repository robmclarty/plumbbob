...
5. [ ] chore(evals): re-measure c5 both arms at 0.10.0, land the receipt, **done when:** `docs/evals/2026-08-14.md` is committed carrying c5 baseline and latched rows at n=5 ([D10 (both-arms)](#d10)), stamped plumbbob 0.10.0, generated by `--report` ([D4 (derived-receipt)](#d4)); the JSONL ledgers stay local in `reports/evals/` (receipt dated by the UTC run day, which crossed to 08-14 before the sweep ran)
