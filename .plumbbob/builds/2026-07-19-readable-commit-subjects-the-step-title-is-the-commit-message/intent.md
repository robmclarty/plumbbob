...
1. [ ] feat(commit-subject): fall back through step scope, build default, then slug —
   **done when:** `subjectForStep` resolves the scope as title-scope → `**Scope:**`
   ...
   type default, and back-compat (no `**Scope:**` ⇒ today's slug behavior, unchanged); the
   `chore(scope): plan` and `chore(scope): finish` subjects use the same default; full
   check green
...
2. [ ] docs(intent-template): make the step title double as the commit subject —
   **done when:** `templates/intent.md` carries the `**Scope:**` header field with its
   ...
   title, and how the scope fallback resolves (D3); full check green
...
3. [ ] feat(pb-plan): author titles as commit subjects and set the build scope —
   **done when:** the Steps-authoring guidance instructs a plain `type(scope):
   description` title with paths kept in `seam`, and directs setting the `**Scope:**`
   default plus per-step overrides; a contract pin matches the new guidance; suite green
...
5. [ ] feat(pb-refine): resync the commit subject when the plan drifts —
   **done when:** repair mode's guidance names the title-subject as something to bring back
   ...
   - model: opus — skill prose sets future authoring behavior
6. [ ] feat(pb-tick): let the checkpoint body pass reconcile a drifted subject —
   **done when:** `/pb-build` and `/pb-verify` document that the body-authoring pass may
   reconcile a genuinely-drifted subject via `-m` (deterministic title-derived subject
   stays the default, D6); contract pins for both match; suite green
   - seam: `skills/pb-build/SKILL.md`, `skills/pb-verify/SKILL.md`,
     `test/contract/skills.test.ts`
   - model: opus — skill prose sets future authoring behavior
7. [ ] docs(commit-grammar): record the title-is-subject rule and the D68 amendment —
